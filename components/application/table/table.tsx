...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("overflow-hidden rounded-xl bg-primary shadow-sm ring-1 ring-secondary", className)}>
                {children}
            </div>
        ...
        <AriaRow
            ...
            className={(state) =>
                cx(
                    ...
                    // Row border—using an "after" pseudo-element to avoid the border taking up space.
                    // "[&>td]:after:absolute [&>td]:after:inset-x-0 [&>td]:after:bottom-0 [&>td]:after:h-px [&>td]:after:w-full [&>td]:after:bg-border-secondary last:[&>td]:after:hidden [&>td]:focus-visible:after:opacity-0",
...
const TableCell = ({ className, children, ...props }: TableCellProps) => {
    ...
    return (
        <AriaCell
            ...
            className={(state) =>
                cx(
                    "relative text-base text-tertiary outline-focus-ring focus-visible:z-1 focus-visible:outline-2 focus-visible:-outline-offset-2",
                    size === "sm" && "px-5 py-3",
                    size === "md" && "px-6 py-4",
